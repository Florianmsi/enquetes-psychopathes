        ...
        .then(snapshot => {
            if (snapshot.exists()) {
                ...
                Object.keys(rawBonus).forEach(k => {
                    // Compatibilité ancien format {selected:"A"} et nouveau format "A"
                    const val = rawBonus[k];
                    bonusAnswers[parseInt(k)] = (typeof val === 'string') ? val : (val && val.selected ? val.selected : null);
                });
            } else {
                userScores = {};
                quizAnswers = {};
...
function isBonusCorrect(answer, index) {
    // answer est directement une lettre "A", "B" ou "C" (string simple)
    if (!answer || typeof answer !== 'string') return false;
    return answer === BONUS_CONFIG[index].correct;
}
...
function renderBonusLines() {
    ...
    BONUS_CONFIG.forEach((bonus, index) => {
        const answered = bonusAnswers[index]; // string "A","B","C" ou undefined
...
function updateBonusSummary() {
    ...
    const items = BONUS_CONFIG.map((bonus, i) => {
        const answered = bonusAnswers[i]; // string ou undefined
...
function applyBonusAnswer(index, letter) {
    const expectedLetter = BONUS_CONFIG[index].correct;
    const correct = (letter === expectedLetter);
    
    // DEBUG - à retirer après confirmation
    alert("DEBUG applyBonusAnswer:\nIndex: " + index + "\nLettre choisie: " + letter + "\nLettre correcte: " + expectedLetter + "\nCorrect: " + correct);
    
    // Stocker dans la variable locale immédiatement
    bonusAnswers[index] = letter;
    
    // Stocker dans Firebase
    firebase.database().ref("users/" + currentUser + "/bonusAnswers/" + index).set(letter)
    .then(function() {
        alert("DEBUG Firebase OK - bonusAnswers[" + index + "] = " + bonusAnswers[index] + "\ngetBonusScore() = " + getBonusScore());
        renderBonusLines();
        updateScoreDisplay();
        updateLeaderboard();
    })
    .catch(function(err) {
        alert("DEBUG Firebase ERREUR: " + err.message);
    });
}
