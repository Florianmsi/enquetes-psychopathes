    ...
    firebase.database().ref('users/' + currentUser).once('value')
        .then(snapshot => {
            if (snapshot.exists()) {
                ...
                // Lire les réponses bonus depuis scores/bonusX
                bonusAnswers = {};
                const sc = userData.scores || {};
                [0,1,2,3].forEach(i => {
                    if (sc["bonus" + i]) bonusAnswers[i] = sc["bonus" + i];
                });
            } else {
                userScores = {};
                quizAnswers = {};
        ...
        .then(snapshot => {
            ...
            snapshot.forEach(childSnapshot => {
                ...
                if (pseudo === currentUser) {
                    ...
                    // Lire les réponses bonus depuis scores/bonusX
                    bonusAnswers = {};
                    const sc2 = userData.scores || {};
                    [0,1,2,3].forEach(i => {
                        if (sc2["bonus" + i]) bonusAnswers[i] = sc2["bonus" + i];
                    });
                }
            });
            updateLeaderboard();
...
function applyBonusAnswer(index, letter) {
    ...
    // Stocker dans scores/bonusX — même nœud que les chapitres, droits garantis
    const key = "bonus" + index;
    firebase.database().ref("users/" + currentUser + "/scores/" + key).set(letter)
    .then(function() {
        renderBonusLines();
        updateScoreDisplay();
        updateLeaderboard();
    ...
    .catch(function(err) {
        console.error("Erreur sauvegarde bonus:", err);
    });
}
