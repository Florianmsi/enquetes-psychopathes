...
// Fonction pour gérer l'animation d'intro avec empreintes digitales
function initIntroAnimation() {
    ...
    // Après 2.5 secondes, changer le texte
    setTimeout(() => {
        scanText.textContent = 'IDENTIFICATION RÉUSSIE';
    }, 2500);
    
    // Faire disparaître l'animation au scroll ou après 5 secondes
    ...
    // Ou automatiquement après 5 secondes
    setTimeout(hideIntro, 5000);
    
    // Cacher aussi au clic/touch
    introAnimation.addEventListener('click', hideIntro, { once: true });
    introAnimation.addEventListener('touchstart', hideIntro, { once: true });
}
...
function createChapterCard(chapter) {
    ...
    // updateChapterStats(chapter.number); // Supprimé car la moyenne n'est plus affichée
