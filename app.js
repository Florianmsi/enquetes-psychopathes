        ...
        .then(snapshot => {
            if (snapshot.exists()) {
                ...
                Object.keys(rawBonus).forEach(k => {
                    bonusAnswers[parseInt(k)] = rawBonus[k];
                });
            } else {
                userScores = {};
                quizAnswers = {};
...
function getBonusScore() {
    // CORRECTION : on vérifie les clés 0,1,2,3 directement
    return BONUS_CONFIG.reduce((total, bonus, i) => {
        const answer = bonusAnswers[i];
...
function renderBonusLines() {
    ...
    BONUS_CONFIG.forEach((bonus, index) => {
        ...
        lineDiv.className = 'bonus-line' + (answered ? (answered.correct ? ' correct' : ' wrong') : '');

        // Ligne horizontale : label + boutons A B C + points
        ...
        // Points bonus — affichés à côté du bouton C, seulement si bonne réponse validée
        const pointsDiv = document.createElement('div');
        pointsDiv.className = 'bonus-points-reveal';
        pointsDiv.textContent = `🏅 +${bonus.points} pt${bonus.points > 1 ? 's' : ''} !`;
        if (answered && answered.correct === true) {
...
function applyBonusAnswer(index, letter) {
    ...
    // CORRECTION : stocker avec clé entière
...
function checkBonusTrophy() {
    ...
    const allCorrect = BONUS_CONFIG.every((_, i) => bonusAnswers[i] && bonusAnswers[i].correct === true);
