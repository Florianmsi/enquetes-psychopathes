...
function loadUserData() {
    alert('loadUserData démarrée');
    const user = getUser(currentUser.pseudo);
    alert('User récupéré: ' + JSON.stringify(user.chapters));
    let total = 0;
    ...
    for (let i = 1; i <= 7; i++) {
        ...
        if (score !== undefined) {
            ...
            alert('Chapitre ' + i + ': affichage de "' + text + '"');
            
            if (el) {
                el.textContent = text;
                el.style.color = '#27ae60';
                alert('✓ Score ' + i + ' affiché');
            } else {
                alert('✗ Element score' + i + ' INTROUVABLE !');
            }
            ...
            if (score === CHAPTERS[i].max) {
                alert('Badge parfait pour chapitre ' + i);
...
function checkBonusUnlock() {
    ...
    alert('Chapitres complétés: ' + completed + '/7');
    alert('Tous complétés? ' + allCompleted);
...
function validateScore() {
    alert('validateScore appelée');
    alert('selectedScore = ' + selectedScore);
    alert('currentChapter = ' + currentChapter);
    
    if (selectedScore === null) {
        alert('ERREUR: selectedScore est null');
        return;
    }
    
    alert('Sauvegarde du score...');
    saveScore(currentUser.pseudo, currentChapter, selectedScore);
    
    alert('Score sauvegardé. Fermeture modal...');
    closeModal();
    
    // Animation score parfait
    const maxScore = CHAPTERS[currentChapter].max;
    alert('Score max: ' + maxScore + ', Score validé: ' + selectedScore);
    
    if (selectedScore === maxScore) {
        alert('SCORE PARFAIT DÉTECTÉ ! Animation...');
        showPerfectAnimation(currentChapter);
    }
    
    alert('Rechargement des données...');
    loadUserData();
}

function showPerfectAnimation(chNum) {
    alert('showPerfectAnimation appelée pour chapitre ' + chNum);
    const item = document.getElementById('ch' + chNum);
    
    if (!item) {
        alert('ERREUR: Element ch' + chNum + ' INTROUVABLE !');
        return;
    }
    
    alert('Création de l\'overlay...');
    ...
    alert('Overlay ajouté ! Animation visible pendant 3 secondes');
