...
function saveScore(pseudo, chapter, score) {
    const users = getAllUsers();
    if (users[pseudo]) {
        // ✅ CORRECTION: Utiliser String(chapter) pour la clé
...
function loadUserData() {
    ...
    // Chapitres 1-7
    for (let i = 1; i <= 7; i++) {
        // ✅ CORRECTION: Utiliser String(i) pour la clé
        ...
        debugLog('Score récupéré: ' + (score !== undefined ? score : 'aucun'));
...
function openChapter(num) {
    ...
    const user = getUser(currentUser.pseudo);
    // ✅ CORRECTION: Utiliser String(num) pour la clé
