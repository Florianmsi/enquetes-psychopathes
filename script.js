...
function loadUserData() {
    ...
    for (let i = 1; i <= 7; i++) {
        ...
        debugLog('--- Chapitre ' + i + ' ---');
        debugLog('Score: ' + (score !== undefined ? score : 'aucun'));
...
function checkBonusUnlock() {
    debugLog('🔓 checkBonusUnlock démarrée');
    const user = getUser(currentUser.pseudo);
    ...
    for (let i = 1; i <= 7; i++) {
        if (user.chapters[i] !== undefined) {
            completed++;
            debugLog('✅ Chapitre ' + i + ': complété');
        } else {
            allCompleted = false;
            debugLog('⚪ Chapitre ' + i + ': non complété');
        }
    }
    
    debugLog('📊 Chapitres complétés: ' + completed + '/7');
    debugLog('🔐 Tous complétés? ' + allCompleted);
    
    const bonusItem = document.getElementById('chBonus');
    const btn = document.getElementById('btnBonus');
    const msg = bonusItem ? bonusItem.querySelector('.locked-msg') : null;
    
    if (!bonusItem) {
        debugLog('❌ ERREUR: Element chBonus INTROUVABLE !');
        return;
    }
    
    if (allCompleted) {
        debugLog('🎉 DÉBLOCAGE DU BONUS !');
        bonusItem.classList.remove('locked');
        bonusItem.classList.add('unlocked');
        btn.disabled = false;
        if (msg) msg.style.display = 'none';
        debugLog('✅ Bonus débloqué avec succès');
    } else {
        debugLog('🔒 Bonus reste verrouillé');
