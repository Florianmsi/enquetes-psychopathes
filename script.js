...
function loadUserData() {
    ...
    debugLog('currentUser.pseudo = ' + currentUser.pseudo);
    
    // Récupérer directement depuis localStorage pour être sûr d'avoir les données fraîches
    const allUsers = getAllUsers();
    debugLog('Tous les users dans localStorage: ' + JSON.stringify(allUsers));
    
    const user = allUsers[currentUser.pseudo];
    if (!user) {
        debugLog('❌ ERREUR CRITIQUE: Utilisateur "' + currentUser.pseudo + '" introuvable!');
        return;
    }
    
    debugLog('✅ User trouvé: ' + JSON.stringify(user));
    debugLog('📦 Chapters du user: ' + JSON.stringify(user.chapters));
    debugLog('🔑 Clés des chapters: ' + Object.keys(user.chapters).join(', '));
    ...
    for (let i = 1; i <= 7; i++) {
        ...
        debugLog('--- Chapitre ' + i + ' ---');
        debugLog('Clé recherchée: "' + String(i) + '"');
        debugLog('Score brut récupéré: ' + score + ' (type: ' + typeof score + ')');
        debugLog('Score !== undefined: ' + (score !== undefined));
        debugLog('Score !== null: ' + (score !== null));
        debugLog('Element score' + i + ' trouvé: ' + (el ? 'OUI' : 'NON'));
        
        // Vérifier si le score existe ET n'est pas null
        if (score !== undefined && score !== null) {
            const text = score + '/' + CHAPTERS[i].max;
            debugLog('📝 Texte à afficher: "' + text + '"');
            
            if (el) {
                el.textContent = text;
                el.style.color = '#27ae60';
                debugLog('✅ innerHTML de l\'élément après modification: ' + el.parentElement.innerHTML);
            } else {
                debugLog('❌ ERREUR: Element score' + i + ' INTROUVABLE !');
            }
            
            total += Number(score);
            debugLog('Total cumulé: ' + total);
            ...
            debugLog('⚠️ Pas de score pour ce chapitre');
            if (el) {
                el.textContent = '-';
                el.style.color = '';
            }
    ...
    const totalEl = document.getElementById('playerScore');
    totalEl.textContent = total;
    debugLog('Score total affiché dans élément: ' + totalEl.textContent);
...
function validateScore() {
    ...
    // Mise à jour immédiate du score dans le DOM
    const scoreEl = document.getElementById('score' + currentChapter);
    if (scoreEl) {
        scoreEl.textContent = selectedScore + '/' + CHAPTERS[currentChapter].max;
        scoreEl.style.color = '#27ae60';
        debugLog('✅ Score affiché directement dans le DOM: ' + scoreEl.textContent);
    } else {
        debugLog('❌ ERREUR: Element score' + currentChapter + ' non trouvé !');
    }
    ...
    setTimeout(() => {
        loadUserData();
        debugLog('✅ validateScore terminée - Données rechargées');
    }, 100);
}
