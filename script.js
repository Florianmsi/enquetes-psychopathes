...
function loadUserData() {
    ...
    for (let i = 1; i <= 7; i++) {
        ...
        console.log(`Chapitre ${i}: score=${score}, element=`, scoreElement, 'card=', chapterCard);
        
        if (score !== undefined) {
            scoreElement.textContent = `${score}/${CHAPTERS_CONFIG[i].max}`;
            scoreElement.classList.add('completed');
            playerTotal += score;
            
            console.log(`Score ${i} mis à jour: ${score}/${CHAPTERS_CONFIG[i].max}`);
            
            const percentage = (score / CHAPTERS_CONFIG[i].max) * 100;
            progressElement.style.width = `${percentage}%`;
            
            // Vérifier si c'est un score parfait et ajouter le badge
            if (score === CHAPTERS_CONFIG[i].max) {
    ...
    console.log('Score total calculé:', playerTotal);
    console.log('Détail: chapitres =', Object.values(userData.chapters), 'bonus =', bonusScore);
    
    document.getElementById('playerScore').textContent = playerTotal;
    document.getElementById('avgScore').textContent = averages.total;
...
function validateChapter() {
    ...
    console.log(`Chapitre ${currentChapter} validé avec ${selectedScore} points. Score max: ${config.max}`);
    
    if (isPerfectScore) {
        console.log('Score parfait détecté ! Lancement de l\'animation...');
        setTimeout(() => {
