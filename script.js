...
function saveScore(pseudo, chapter, score) {
    const users = getAllUsers();
    if (users[pseudo]) {
        // IMPORTANT: Utiliser un nombre comme clé, pas une string
        users[pseudo].chapters[chapter] = Number(score);
        saveAllUsers(users);
        debugLog('💾 Sauvegarde: chapters[' + chapter + '] = ' + score);
    }
}
...
function loadUserData() {
    ...
    // Chapitres 1-7
    for (let i = 1; i <= 7; i++) {
        // CORRECTION: Utiliser directement le nombre comme clé
...
function openChapter(num) {
    ...
    const user = getUser(currentUser.pseudo);
    const currentScore = user.chapters[num];
